...
## based on q4, we should use relu activation
## based on question 2, relu dnn with 30+ layers had a fall off in auc, maybe translates to rmse?


class dnnNet(nn.Module):
    def __init__(self, n_layers, n_neurons):
        super().__init__()
        #self.dropout = nn.Dropout(0.2) #added
        self.layers = nn.ModuleList(
...
#dataset = TensorDataset(X_train_b, y_train_b)
...
# #log
# # starts with preliminary 2 layers, 64, 128 or 256 neurons. lr of 1e-3 resulted in rmse increasing as
# # neurons increased, so lowered to 1e-4. after that, we increase the epoch to account for the longer learning time
# # and then only test 256 neurons with 2,4,6 layers (since 256 is clearly better than less).
# #RMSE slightly increased at 1000 epochs, suggesting there is overfitting going on 
# # changing it back to 500: still rmse increasing with more layers, try adding dropouts
# # added mini batch gd, but this actually decreases rmse
# #trying SGD instead
# # nope, not workng.  network closest to achieving the single layer quality was a 2 layer network with 256 neurons, 
#which was a slight improvement (could be noise/ due to random seeding ?)
# # at an RMSE of 6.085 instead of 6.089
...
plt.title('Permutation Importance - DNN (6 layers, ReLU)')
